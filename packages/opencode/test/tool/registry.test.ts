...
describe("tool.registry", () => {
  ...
  it.instance("offers web video generation only when a NanoGPT key is configured", () => {
    ...
    return Effect.gen(function* () {
      ...
      expect(yield* ids()).not.toContain("generate_web_video")
      process.env.NANOGPT_API_KEY = "sk-nano-test-0123456789"
      expect(yield* ids()).toContain("generate_web_video")
    }).pipe(
      Effect.ensuring(
        Effect.sync(() => {
          if (previous === undefined) delete process.env.NANOGPT_API_KEY
          if (previous !== undefined) process.env.NANOGPT_API_KEY = previous
        }),
      ),
    )
  })
