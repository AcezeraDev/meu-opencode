  ...
  return (
    ...
    >
      ...
      <Show when={props.newLayout}>
        <ScopeTrace active={!!props.working} />
      </Show>
    </div>
  )
}
...
export default function Page() {
  ...
  // Regenerate/Retry on a web video card pre-fill the composer; the user sends it.
  makeEventListener(window, COMPOSER_FILL_EVENT, (event) => {
    const text = (event as CustomEvent<ComposerFillDetail>).detail?.text
    if (!text) return
    prompt.set([{ type: "text", content: text, start: 0, end: text.length }], text.length)
    requestAnimationFrame(() =>
      document
        .querySelector<HTMLElement>(
          '[data-component="prompt-input-v2"] [contenteditable], [data-component="prompt-input"] [contenteditable]',
        )
        ?.focus(),
    )
  })
  ...
  const sessionMode = createSessionModeState()
  const titlebarRight = useTitlebarRightMount()
  // Review focus only hides the chat while the review panel is actually open, so
  ...
  const applySessionMode = (mode: SessionMode) => {
    ...
    if (mode === "cockpit") {
      ...
      view().terminal.close()
      const available = sessionPanelAvailable()
      if (available) layout.session.resize(Math.round(available * 0.58))
      return
    }
    if (mode === "split") {
      view().reviewPanel.open()
              ...
              promptInput={
                ...
                >
                  {(_) => {
                    ...
                    return (
                      <PromptInputV2Composer controller={controller} borderUnderlay contextControl={<ContextRing />} />
                    )
                  }}
                </Show>
              }
  ...
  return (
    <SessionRouteFrame>
      <SessionHeader />
      <Show when={newSessionDesign() && isDesktop() && !!params.id && titlebarRight()} keyed>
        {(mount) => (
          <Portal mount={mount}>
            <SessionModeSwitcher mode={sessionMode.mode()} onChange={applySessionMode} />
          </Portal>
        )}
      </Show>
      <div class="flex-1 min-h-0 flex flex-row">
        <Show when={cockpitRail()}>
          <SessionRail current={params.id} />
        </Show>
        <div
          ref={panelRow}
        ...
        >
          ...
          >
            ...
            <Show when={desktopSessionResizeOpen() && !reviewFocus()}>
