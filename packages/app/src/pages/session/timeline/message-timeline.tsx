...
export function MessageTimeline(props: {
  ...
  // Output that arrives while the agent works settles in once; rows scrolled back
  // into view later (virtualization remounts them) appear without replaying it.
  const seenRows = new Set<string>()
  const markLiveRow = (row: TimelineRow.TimelineRow) => {
    const key = TimelineRow.key(row)
    const live = !seenRows.has(key) && workingTurn(row.userMessageID)
    seenRows.add(key)
    return live
  }

  /** A summary shown right as its response finished settles in instead of just being there. */
  const freshTurn = (userMessageID: string) => {
    const completed = sessionMessages().flatMap((message) =>
      message.role === "assistant" && message.parentID === userMessageID ? [message.time.completed ?? 0] : [],
    )
    return completed.length > 0 && Date.now() - Math.max(...completed) < 4000
  }

  const userPreview = (userMessageID: string) =>
    getMsgParts(userMessageID)
  ...
  const overviewMarkers = createMemo<OverviewMarker[]>(() =>
    timelineRows().flatMap((row, index): OverviewMarker[] => {
      ...
      if (row._tag === "Thinking") return [{ index, kind: "live", label: language.t("scope.overview.live") }]
      if (row._tag !== "AssistantPart" || row.group.type !== "part") return []
      const part = getMsgPart(row.group.ref.messageID, row.group.ref.partID)
      if (part?.type !== "tool" || !["edit", "write", "apply_patch"].includes(part.tool)) return []
      const target = toolTarget(part)
      return [
        {
          index,
          kind: "edit",
          label: target ? `${language.t("scope.overview.edit")} · ${target}` : language.t("scope.overview.edit"),
        },
      ]
    }),
  )
  ...
  const renderTimelineRow = (row: Accessor<TimelineRow.TimelineRow>, onSizeChange?: () => void) => {
    switch (row()._tag) {
      ...
      case "AssistantPart": {
        ...
        const live = markLiveRow(assistantPartRow())
        return (
          <TimelineRowFrame row={assistantPartRow}>
            <div data-slot="session-turn-message-container" class="w-full px-4 md:px-5">
              <div
                data-slot="session-turn-assistant-content"
                data-scope-live={live ? "" : undefined}
      ...
      case "TurnStats": {
        const turnStatsRow = row as Accessor<TimelineRowByTag<"TurnStats">>
        return (
          <TimelineRowFrame row={turnStatsRow}>
            <div data-slot="session-turn-message-container" class="w-full px-4 md:px-5">
              <TurnStats
                messages={sessionMessages()}
