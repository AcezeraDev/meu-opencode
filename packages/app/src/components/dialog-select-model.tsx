...
function ModelSelectorPopoverV2View(props: {
  ...
  const activeModel = createMemo(
    () =>
      models().find((item) => modelKey(item) === store.active) ??
      models().find((item) => modelKey(item) === props.current()),
  )
    ...
    <MenuV2 open={store.open} modal={false} placement="top-start" gutter={6} onOpenChange={setOpen}>
      ...
      <MenuV2.Portal>
        <MenuV2.Content
          ...
          class="flex !flex-row w-[284px] overflow-hidden rounded-[16px] border-0 bg-v2-background-bg-layer-01 !p-0 shadow-[var(--v2-elevation-floating)] focus:outline-none sm:w-[584px]"
          onPointerDownOutside={dismiss.preventTriggerRestore}
          onFocusOutside={dismiss.preventTriggerRestore}
          onCloseAutoFocus={dismiss.onCloseAutoFocus}
        >
          <div class="flex w-[284px] min-w-0 shrink-0 flex-col">
            ...
            <ScrollView data-slot="model-selector-scroll" class="max-h-[300px] min-h-0">
          ...
          <div class="hidden min-w-0 flex-1 flex-col border-l border-v2-border-border-muted p-4 sm:flex">
            <Show when={activeModel()} keyed>
              {(item) => (
                <ModelTooltip model={item} latest={item.latest} free={isFree(item.provider.id, item.cost)} card />
              )}
            </Show>
          </div>
        </MenuV2.Content>
