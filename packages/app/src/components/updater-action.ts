...
export function updaterAction(state: UpdaterState | undefined, personal = false) {
  ...
  switch (state.status) {
    ...
    case "downloading":
      return personal
        ? { label: "settings.updates.action.building" as const }
...
export function useUpdaterAction() {
  ...
  const action = createMemo(() => updaterAction(platform.updater?.state(), platform.updater?.personal))
