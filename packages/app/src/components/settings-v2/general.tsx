...
import { useSettings, type ChromaSpeed } from "@/context/settings"
...
const ScopeSettings = () => {
  ...
  return (
    <>
      ...
      >
        <div data-action="settings-chroma">
          <Switch
            checked={settings.appearance.chroma()}
            onChange={(checked) => settings.appearance.setChroma(checked)}
          />
        </div>
      </SettingsRowV2>

      <SettingsRowV2
        title={language.t("settings.general.row.chromaSpeed.title")}
        description={language.t("settings.general.row.chromaSpeed.description")}
      >
        <SelectV2
          appearance="inline"
          data-action="settings-chroma-speed"
          options={chromaSpeeds}
          current={settings.appearance.chromaSpeed()}
          placement="bottom-end"
          gutter={6}
          label={(option) => language.t(`settings.general.row.chromaSpeed.${option}`)}
          onSelect={(option) => option && settings.appearance.setChromaSpeed(option)}
        />
      </SettingsRowV2>

      <SettingsRowV2
        title={language.t("settings.general.row.chromaGlow.title")}
        description={language.t("settings.general.row.chromaGlow.description")}
      >
        <div data-action="settings-chroma-glow">
          <Switch
            checked={settings.appearance.chromaGlow()}
            onChange={(checked) => settings.appearance.setChromaGlow(checked)}
          />
        </div>
      </SettingsRowV2>

      <SettingsRowV2
        title={language.t("settings.general.row.dailyLimit.title")}
        description={language.t("settings.general.row.dailyLimit.description")}
      >
        <div class="w-full sm:w-[120px]">
          <TextInputV2
            data-action="settings-daily-limit"
            type="number"
            appearance="base"
            min="0"
            step="0.5"
            value={settings.usage.dailyLimit() ? String(settings.usage.dailyLimit()) : ""}
            placeholder="0"
            onChange={(event) => settings.usage.setDailyLimit(Number(event.currentTarget.value))}
            aria-label={language.t("settings.general.row.dailyLimit.title")}
          />
        </div>
      </SettingsRowV2>
    </>
