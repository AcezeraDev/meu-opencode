...
import "./model-tooltip.css"

type InputKey = "text" | "image" | "audio" | "video" | "pdf"
type InputMap = Record<InputKey, boolean>

const INPUT_KEYS: Array<InputKey> = ["text", "image", "audio", "video", "pdf"]
...
export const ModelTooltip: Component<{
  ...
  const hasInput = (key: InputKey) =>
    props.model.capabilities ? props.model.capabilities.input[key] : (props.model.modalities?.input ?? []).includes(key)

  if (props.card) {
    return (
      <div class="model-card-v2">
        <Show when={props.model.provider.id}>
          {(id) => (
            <span class="model-card-v2-tile">
              <ProviderIcon id={id()} width={20} height={20} />
            </span>
          )}
        </Show>
        <div class="model-card-v2-title">
          <span class="model-card-v2-name">{props.model.name}</span>
          <span class="model-card-v2-provider">{props.model.provider.name}</span>
        </div>
        <Show when={props.latest || props.free}>
          <div class="model-card-v2-tags">
            <Show when={props.latest}>
              <span class="model-card-v2-tag" data-tone="accent">
                {language.t("model.tag.latest")}
              </span>
            </Show>
            <Show when={props.free}>
              <span class="model-card-v2-tag" data-tone="success">
                {language.t("model.tag.free")}
              </span>
            </Show>
          </div>
        </Show>
        <div class="model-card-v2-facts">
          <div class="model-card-v2-fact">
            <span class="model-card-v2-label">{language.t("model.tooltip.context.label")}</span>
            <span class="model-card-v2-value" data-numeric="">
              {contextLimit()}
            </span>
          </div>
          <div class="model-card-v2-fact">
            <span class="model-card-v2-label">{language.t("model.tooltip.reasoning")}</span>
            <span class="model-card-v2-value">{reasoning()}</span>
          </div>
        </div>
        <span class="model-card-v2-label">{language.t("model.tooltip.inputs")}</span>
        <div class="model-card-v2-inputs">
          <For each={INPUT_KEYS}>
            {(key) => (
              <span class="model-card-v2-input" data-enabled={hasInput(key) ? "" : undefined}>
                {inputLabel(key)}
              </span>
            )}
          </For>
