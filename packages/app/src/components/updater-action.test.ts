...
describe("updaterAction", () => {
  ...
  test("personal builds compile instead of downloading", () => {
    expect(updaterAction({ status: "downloading", version: "" }, true)).toEqual({
      label: "settings.updates.action.building",
    })
    expect(updaterAction({ status: "idle" }, true)).toEqual({ label: "settings.updates.action.checkNow", run: "check" })
  })
})
